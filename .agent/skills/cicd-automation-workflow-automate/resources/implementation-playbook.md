...
```json
{
  "extends": ["config:base", ":dependencyDashboard", ":semanticCommits", ":automergeDigest", ":automergeMinor"],
...
export class WorkflowOrchestrator extends EventEmitter {
  ...
  private async executeStep(step: WorkflowStep, result: WorkflowResult, parentPath: string = ""): Promise<void> {
  ...
  private async executeParallel(steps: WorkflowStep[], result: WorkflowResult, parentPath: string): Promise<void> {
    await Promise.all(steps.map((step) => this.executeStep(step, result, parentPath)));
  }

  private async executeSequential(steps: WorkflowStep[], result: WorkflowResult, parentPath: string): Promise<void> {
