...
**Note:** These are agent TYPE definitions, not a fixed count. Loki Mode dynamically spawns agents based on project
needs - a simple todo app might use 5-10 agents, while a complex startup could spawn 100+ agents working in parallel.
...
Location: `.loki/state/agents/{AGENT_ID}.json` Update after every task completion.
```
