...
3. **Fetch & Analyze Reference URL:** Validate variables against a real page.
   ...
   - **Action (REQUIRED):** Use `WebFetch` or a browser DOM snapshot to retrieve page content before choosing any
     selector.
...
5. **Verify Variables:** Ensure the chosen variables (Preset, Schema, Selector) exist in your analysis.
   - **Action (REQUIRED):** If a selector cannot be verified from the fetched content, state that explicitly and ask for
     another URL.
...
- **Never guess selectors.** If the DOM cannot be accessed or the element is missing, ask for another URL or a
  screenshot.
