...
```javascript
const thresholdScale = d3.scaleThreshold().domain([0, 10, 20]).range(["freezing", "cold", "warm", "hot"]);
...
// Custom range
const customScale = d3.scaleOrdinal().domain(["cat1", "cat2", "cat3"]).range(["#FF6B6B", "#4ECDC4", "#45B7D1"]);
```
...
```javascript
const scale = d3.scaleLinear().domain([0, 100]).range(["blue", "red"]).interpolate(d3.interpolateHsl);
...
```javascript
const scale = d3.scaleLinear().domain([0, 100]).range(["blue", "red"]).interpolate(d3.interpolateLab);
...
```javascript
const scale = d3.scaleLinear().domain([0, 100]).range(["blue", "red"]).interpolate(d3.interpolateHcl);
