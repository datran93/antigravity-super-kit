...
### Phase 3: Execution & Acceptance Analysis ▶️
...
- **MANDATORY**: Capture and analyze the logs for any failures. Do not assume pass based on heuristics.

### Phase 4: Feedback, Resolution & Drift Detection 📝

Act on the test results.

- **PASS**: If all tests pass, and coverage is >= 70%, summarize the coverage and confirm stability. Transfer to
  `planner` role and mark the overall step as complete via `@mcp:context-manager` (`complete_task_step`), passing in the
  `active_files` array.
...
- **Panic Protocol (Drift Detection)**: Immediately call `@mcp:context-manager` (`record_failure`) if a test fails
  ...
  **3 times**), ABORT the `coder` ↔ `tester` loop immediately to prevent token wastage. Transition straight to the
  `[Role: 🏗️ Planner]` and trigger Socratic questioning with the USER. Do not bounce back to Coder.
