          ...
          <div className="flex h-full">
            ...
            <div className="w-80 lg:w-96 border-r border-gray-800/50 flex flex-col shrink-0">
              ...
              <div className="flex-1 overflow-y-auto">
                ...
                {tasks.length === 0 && (
                  <div className="p-6 text-center text-gray-600 text-sm">No tasks found.</div>
                )}
              </div>
            </div>
                ...
                <div className="flex-1 overflow-y-auto p-6">
                  ...
                  {selectedTask.acceptance_criteria && (
                    <div className="bg-gray-900/80 rounded-lg p-4 border border-gray-800 mb-6">
                      <h3 className="text-xs font-semibold text-gray-500 uppercase mb-2 tracking-wider">Acceptance Criteria</h3>
                      <p className="text-sm text-gray-300 whitespace-pre-wrap leading-relaxed">{selectedTask.acceptance_criteria}</p>
                    </div>
                  )}
                        ...
                        <div className="mb-4">
                          ...
                          <div className="space-y-1.5 ml-5">
                            {pendingSteps.map((step) => (
                              <div key={step.step_id} className="px-3 py-2.5 bg-gray-900/60 border border-gray-800 rounded-md text-sm text-gray-300">
                                {step.name}
                              </div>
                            ))}
                          </div>
                      ...
                      {completedSteps.length > 0 && (
                        <div>
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-xs font-medium text-gray-500">Completed ({completedSteps.length})</span>
                          </div>
                          <div className="space-y-1.5 ml-5">
                            {completedSteps.map((step) => (
                              <div key={step.step_id} className="px-3 py-2.5 bg-gray-900/30 border border-gray-800/50 rounded-md text-sm text-gray-500 line-through">
                                {step.name}
                              </div>
                            ))}
                          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {knowledgeItems.map((ki) => (
                ...
                >
                  ...
                  <p className={`text-sm text-gray-400 mb-3 whitespace-pre-wrap break-words leading-relaxed ${expandedKiId === ki.ki_path ? "" : "line-clamp-3"}`}>
                    {ki.summary}
                  </p>
              ...
              {knowledgeItems.length === 0 && <p className="text-gray-600 col-span-full text-center py-12">No knowledge items indexed.</p>}
            </div>
          </div>
        )}
      </div>
