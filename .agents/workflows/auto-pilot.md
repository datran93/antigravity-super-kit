...
1. **State over Files**: Use the MCP database (`context-manager`) to pass state between roles. DO NOT write or read
   markdown reports during the transition.
...
- **Tester to Completion**: Is coverage >= 70% and are all bugs fixed? → Mark `status = "completed"`, output final
...
3. **Status Update Only**: Output final status only when the entire pipeline is complete or when the Auto-Pilot
