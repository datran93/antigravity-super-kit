...
> **Goal**: Avoid loading the entire `ANCHORS.md` file on every session. Only load the domain-specific constraints that
> are relevant to the current task to save input tokens and improve LLM focus.

### How to Apply

In **Phase 0** of every workflow, instead of reading `ANCHORS.md` as a whole file, the agent MUST:

1. **Classify the task domain** from the user request or checkpoint:
   - `ui` — frontend, styling, components
   - `db` — migrations, schema, queries
   - `security` — auth, payment, secrets, permissions
   - `api` — REST/gRPC contracts, integrations
   - `refactor` — internal code restructuring
2. **Fetch only matching anchor domains** using `manage_anchors(action="list")` and filter by `[domain:X]` tags.
   - `quality` domain is ALWAYS loaded regardless of task type.
   - `codebase` domain is loaded when modifying existing code.
3. **Do NOT read the full file** unless the task explicitly spans 4+ domains (e.g., greenfield project setup).

### Domain → Anchor Mapping Quick Reference

| Task Domain       | Load Anchors                          |
| ----------------- | ------------------------------------- |
| UI / frontend     | `stack`, `quality`, `codebase`        |
| DB / migration    | `db`, `quality`, `git`, `security`    |
| Auth / security   | `security`, `quality`, `git`          |
| API / integration | `stack`, `security`, `quality`, `git` |
| Refactor          | `codebase`, `quality`, `git`          |
| _(always)_        | `quality`                             |
